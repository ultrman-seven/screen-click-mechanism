...
com = 0
# class FoobarTest():
    # def __init__( COM_NAME='COM3'):
def open(COM_NAME):
    try:
        global com
        com = serial.Serial(COM_NAME, 115200, 5)
        return True
...
@send_serial_CMD
def click():
    com.write('\x01\x03'.encode())

@send_serial_CMD
def move( x, y):
    bytes_data_x = struct.pack('<f', x)
    bytes_data_y = struct.pack('<f', y)
    com.write('\x09\x02'.encode())
    com.write(bytes_data_x)
    com.write(bytes_data_y)

@send_serial_CMD
def setServo_X_Y_Position( id, x, y):
    bytes_data_x = struct.pack('<f', x)
    bytes_data_y = struct.pack('<f', y)
    if (id == 0):
        com.write('\x09\x05'.encode())
    else:
        com.write('\x09\x06'.encode())
    com.write(bytes_data_x)
    com.write(bytes_data_y)

@send_serial_CMD
def setLinkLength( id, l1, l2):
    bytes_data_x = struct.pack('<f', l1)
    bytes_data_y = struct.pack('<f', l2)
    if (id == 0):
        com.write('\x09\x07'.encode())
    else:
        com.write('\x09\x08'.encode())
    com.write(bytes_data_x)
    com.write(bytes_data_y)

@send_serial_CMD
def setClickPosition( off, on):
    bytes_data_x = struct.pack('<f', off)
    bytes_data_y = struct.pack('<f', on)
    com.write('\x09\x0a'.encode())
    com.write(bytes_data_x)
    com.write(bytes_data_y)
...
def setClickTime( t):
    ...
    com.write('\x03\x09'.encode())
    com.write(bytes_data)

@send_serial_CMD
def setServoIncTime( t):
    bytes_data = struct.pack("<H", t)
    com.write('\x03\x0f'.encode())
    com.write(bytes_data)

@send_serial_CMD
def saveCfg():
    com.write('\x01\x0c'.encode())

@send_serial_CMD
def set_servo_position( id, p):
    com.write('\x03\x0d'.encode())
    bytes_data_1 = struct.pack("<B", id)
    bytes_data_2 = struct.pack("<B", p)
    com.write(bytes_data_1)
    com.write(bytes_data_2)

@send_serial_CMD
def move_slow( x,y):
    com.write('\x09\x10'.encode())
